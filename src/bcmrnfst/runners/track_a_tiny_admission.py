...
from bcmrnfst.baselines import (
    BaselineModel,
    fit_baseline,
    predict_dataframe,
    update_baseline_model,
)
from bcmrnfst.continual import (
...
from bcmrnfst.fit import fit_bc_mrnfst
from bcmrnfst.prototype.bank import serialize_projection_meta, serialize_threshold_meta
...
class TrackATinyAdmissionRunResult:
...
TrackAModelState = BCMRNFSTModel | BaselineModel | list[BCMRNFSTModel]


@dataclass(frozen=True, slots=True)
class _TrackABurstPolicy:
    """Effective GONS policy for one burst under the current promotion state."""

    refresh_every_bursts: int


def run_track_a_tiny_admission_experiment(
    config_path: Path,
    *,
    ...
    config = load_track_a_tiny_admission_config(config_path, repo_root=root)
    if seed is not None:
    ...
    episodes = _build_track_a_episodes(
        config,
    ...
    for episode in episodes:
        ...
        for burst_index, burst in enumerate(bursts, start=1):
            ...
            burst_policy = _resolve_track_a_burst_policy(
                config=config,
            ...
            elif isinstance(model_state, BCMRNFSTModel):
            ...
            if _is_bc_track_a_state(model_state) and staged_item_ids:
                should_refresh = (
                    burst_index % burst_policy.refresh_every_bursts == 0
                    or (burst_index == len(bursts) and config.run_final_refresh)
                )

            if should_refresh and _is_bc_track_a_state(model_state):
                _report_progress(
                    progress_callback,
                    "update",
                ...
                if (
                    ...
                    and _track_a_state_has_label(model_state, episode.novel_label)
                ):
                    promotion_burst_index = burst_index
    ...
    aggregate_metrics = _aggregate_track_a_metrics(
    ...
    return TrackATinyAdmissionRunResult(
        method=config.method,
...
def _build_track_a_episodes(
    config: TrackATinyAdmissionExperimentConfig,
    *,
...
def _fit_initial_model(
    *,
    ...
    if config.method == "bc_mrnfst":
        if int(config.ensemble_size) > 1:
            models: list[BCMRNFSTModel] = []
            ...
            for member_index in range(int(config.ensemble_size)):
                ...
                models.append(fit_bc_mrnfst(train, calibration, member_config))
            return models
        model_config = config.model.model_copy(update={"label_column": label_column})
        return fit_bc_mrnfst(train, calibration, model_config)
...
def _is_bc_track_a_state(model_state: TrackAModelState) -> bool:
    return isinstance(model_state, BCMRNFSTModel) or isinstance(model_state, list)


def _track_a_state_has_label(model_state: TrackAModelState, label: str) -> bool:
    if isinstance(model_state, list):
...
def _snapshot_state(model_state: TrackAModelState) -> EvaluationSnapshot:
    if isinstance(model_state, list):
...
def _predict_dataframe_with_status(
    ...
    *,
    ...
    if isinstance(model_state, BCMRNFSTModel):
        predictions = predict_bc_mrnfst_dataframe(model_state, frame)
...
def _predict_bc_ensemble_dataframe(
    ...
    *,
    ...
    member_predictions = [predict_bc_mrnfst_dataframe(model, frame) for model in models]
...
def _resolve_track_a_burst_policy(
    *,
    config: TrackATinyAdmissionExperimentConfig,
    model_state: TrackAModelState,
    novel_label: str,
    ...
    if isinstance(model_state, list):
        promoted = _track_a_state_has_label(model_state, novel_label)
        for member_state in model_state:
            _apply_track_a_policy_overrides(
                config=config,
        ...
        return _TrackABurstPolicy(refresh_every_bursts=refresh_every_bursts)
...
def _apply_track_a_policy_overrides(
    *,
    config: TrackATinyAdmissionExperimentConfig,
    model_state: BCMRNFSTModel,
    promoted: bool,
...
def _aggregate_track_a_metrics(
    *,
...
def _serialize_state_metadata(model_state: TrackAModelState) -> dict[str, object]:
    if isinstance(model_state, list):
        return {
            "calibration_mode": "ensemble",
