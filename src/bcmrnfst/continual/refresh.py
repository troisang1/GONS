...
from bcmrnfst.continual.inference import rescore_unknown_buffer_after_refresh
from bcmrnfst.localization import localize_mutual_rnn_units
from bcmrnfst.preprocess import transform_rows
from bcmrnfst.projection import (
...
from bcmrnfst.prototype import (
    Prototype,
...
def _archive_manifest(model: BCMRNFSTModel) -> ArchiveManifest:
...
def _drop_class_prototypes(model: BCMRNFSTModel, class_label: str) -> None:
    model.proto_bank = {
        proto_id: prototype
...
def _drop_provisional_prototypes(model: BCMRNFSTModel, class_label: str | None = None) -> None:
...
def _support_weight_by_class(model: BCMRNFSTModel, class_label: str) -> float:
    weights = [
...
def _rebuild_calibration_from_support_store(model: BCMRNFSTModel) -> BCMRNFSTModel:
...
def enforce_pending_budget(model: BCMRNFSTModel) -> BCMRNFSTModel:
...
def load_item_payload(model: BCMRNFSTModel, item_id: str) -> PendingPayload:
...
def archive_class_state(model: BCMRNFSTModel, class_label: str) -> BCMRNFSTModel:
...
def load_archived_class_state(model: BCMRNFSTModel, class_label: str) -> dict[str, SupportBundle]:
...
def _shallow_model_copy(model: BCMRNFSTModel) -> BCMRNFSTModel:
...
def build_refresh_sandbox(
    ...
    *,
    ...
    if getattr(sandbox.config, "enable_carag", False):
        from bcmrnfst.projection.carag import fit_carag_gate
        gate_core, gate_tau, carag_state = fit_carag_gate(sandbox)
        # Carry over fit-time β + δ-cal stats (don't have df_cal here).
        prior = getattr(sandbox, "carag_state", None)
        if prior is not None and gate_core is not None:
            carag_state.beta = prior.beta
            carag_state.delta_calib_median = dict(prior.delta_calib_median)
            carag_state.delta_calib_std = dict(prior.delta_calib_std)
        sandbox.dvmad_core = gate_core
        sandbox.dvmad_tau = gate_tau
        sandbox.carag_state = carag_state
    elif getattr(sandbox.config, "enable_dvmad_gate", False):
        from bcmrnfst.projection.dvmad_gate import refit_gate_with_scale
        gate_core, gate_tau = refit_gate_with_scale(sandbox)
        sandbox.dvmad_core = gate_core
        sandbox.dvmad_tau = gate_tau
    if getattr(sandbox.config, "enable_ewm", False):
        from bcmrnfst.projection.dvmad_ewm import fit_ewm
        sandbox.ewm_state = fit_ewm(sandbox)
    if getattr(sandbox.config, "enable_nsd_gate", False):
        from bcmrnfst.projection.dvmad_nsd_gate import fit_nsd_gate
        sandbox.nsd_gate_state = fit_nsd_gate(sandbox)
    if getattr(sandbox.config, "enable_pct_rd", False):
        from bcmrnfst.projection.pct_rd import fit_pct_rd
        # Refresh refit: keep alpha + tau from the original calibration via
        # carry-over (sandbox is a shallow copy of model). Fit fresh per-class
        # cores + g_norm; tau_per_class survives from prior calibration.
        prior = getattr(sandbox, "pct_rd_state", None)
        new_state = fit_pct_rd(sandbox)
        if prior is not None and new_state.cores:
            new_state.alpha_pct = prior.alpha_pct
            new_state.alpha_ncm = prior.alpha_ncm
            # Keep prior tau for known classes; new classes inherit global fallback.
            new_state.tau_per_class = dict(prior.tau_per_class)
            new_state.tau_global = prior.tau_global
        sandbox.pct_rd_state = new_state
...
def _cleanup_refresh_item(model: BCMRNFSTModel, item_id: str) -> None:
