...
from bcmrnfst.config.model import BCMRNFSTConfig, RefreshMode, SupportTrimStrategy
from bcmrnfst.data.ton_iot import normalize_label_value
...
TrackATinyAdmissionMethod: TypeAlias = StaticOpenSetMethod
TrackAFscilMethod: TypeAlias = StaticOpenSetMethod
CollapseTestsMethod: TypeAlias = Literal["bc_mrnfst"]
FidelityStudyMethod: TypeAlias = Literal["bc_mrnfst"]


class StaticOpenSetExperimentConfig(BaseModel):
    """Typed configuration for the S0 static open-set runner."""

    task: Literal["s0_static_open_set"] = "s0_static_open_set"
    experiment_name: str
    method: StaticOpenSetMethod
    ...
    include_leave_multiple_classes_out: bool = True
    leave_multiple_group_size: PositiveInt = 2
    leave_multiple_max_groups: PositiveInt | None = None
    ...
    @classmethod
    def normalize_holdout_groups(cls, value: object) -> list[list[str]]:
        """Normalize and deduplicate explicit holdout groups."""
        ...
        for group in value:
            ...
            if not normalized_group:
                raise ValueError("Holdout groups cannot be empty.")
            if normalized_group not in normalized_groups:
                normalized_groups.append(normalized_group)
        return normalized_groups

    @model_validator(mode="after")
    def validate_holdout_policy(self) -> StaticOpenSetExperimentConfig:
        """Require at least one way to generate holdout episodes."""

        if not self.holdout_groups and not (
            self.include_leave_one_class_out or self.include_leave_multiple_classes_out
        ):
            raise ValueError(
                "Enable at least one generated holdout policy or provide explicit holdout_groups."
            )
        return self
    ...
    def to_baseline_fit_config(self) -> BaselineExperimentConfig:
        """Adapt the S0 config into the shared baseline fit config."""

        if self.method == "bc_mrnfst":
            raise ValueError("bc_mrnfst does not use the baseline config path.")
...
def load_static_open_set_config(
    path: Path,
    *,
    repo_root: Path | None = None,
) -> StaticOpenSetExperimentConfig:
    """Load a static open-set config from YAML."""
    ...
    return StaticOpenSetExperimentConfig.model_validate(payload)


class SessionContinualExperimentConfig(BaseModel):
    """Typed configuration for the S1 session-based continual runner."""

    task: Literal["s1_session_continual"] = "s1_session_continual"
    experiment_name: str
    method: SessionContinualMethod
    dataset_config_path: Path = Path("configs/data/ton_iot.yaml")
    train_path: Path
    calibration_path: Path
    test_path: Path
    ...
    minimum_session_rows: PositiveInt = 1
    ...
    def to_baseline_fit_config(self) -> BaselineExperimentConfig:
        """Adapt the S1 config into the shared baseline fit config."""
...
def load_session_continual_config(
    path: Path,
    *,
    repo_root: Path | None = None,
) -> SessionContinualExperimentConfig:
    """Load an S1 session continual config from YAML."""

    resolved_path = path if path.is_absolute() else (repo_root or find_repo_root()) / path
    payload = yaml.safe_load(resolved_path.read_text(encoding="utf-8")) or {}
    if not isinstance(payload, dict):
        raise ValueError(f"Expected a mapping in {resolved_path}")
    return SessionContinualExperimentConfig.model_validate(payload)


class PrequentialStreamExperimentConfig(BaseModel):
    """Typed configuration for the S2 prequential stream runner."""

    task: Literal["s2_prequential_stream"] = "s2_prequential_stream"
    experiment_name: str
    method: PrequentialStreamMethod
    dataset_config_path: Path = Path("configs/data/ton_iot.yaml")
    train_path: Path
    calibration_path: Path
    ...
    max_stream_batches: PositiveInt | None = None
    minimum_stream_rows: PositiveInt = 1
    label_delay_batches: PositiveInt = 1
    refresh_every_reveals: PositiveInt = 1
    run_refresh_on_final_flush: bool = True
    ...
    def to_baseline_fit_config(self) -> BaselineExperimentConfig:
        """Adapt the S2 config into the shared baseline fit config."""
...
def load_prequential_stream_config(
    path: Path,
    *,
    repo_root: Path | None = None,
) -> PrequentialStreamExperimentConfig:
    """Load an S2 prequential stream config from YAML."""

    resolved_path = path if path.is_absolute() else (repo_root or find_repo_root()) / path
    payload = yaml.safe_load(resolved_path.read_text(encoding="utf-8")) or {}
    if not isinstance(payload, dict):
        raise ValueError(f"Expected a mapping in {resolved_path}")
    return PrequentialStreamExperimentConfig.model_validate(payload)


class TrackATinyAdmissionExperimentConfig(BaseModel):
    """Typed configuration for the FSCIL tiny-admission runner."""

    task: Literal["track_a_tiny_admission"] = "track_a_tiny_admission"
    experiment_name: str
    method: TrackATinyAdmissionMethod
    dataset_config_path: Path = Path("configs/data/ton_iot.yaml")
    train_path: Path
    calibration_path: Path
    test_path: Path
    ...
    @field_validator("holdout_groups", mode="before")
    @classmethod
    def normalize_track_a_holdout_groups(cls, value: object) -> list[list[str]]:
    ...
    @model_validator(mode="after")
    def validate_track_a_policy(self) -> TrackATinyAdmissionExperimentConfig:
        ...
        if self.ensemble_size > 1 and self.method != "bc_mrnfst":
            raise ValueError("FSCIL ensembles are only supported for method=bc_mrnfst.")
        return self
...
def load_track_a_tiny_admission_config(
    path: Path,
    *,
    repo_root: Path | None = None,
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"Expected a mapping in {resolved_path}")
    return TrackATinyAdmissionExperimentConfig.model_validate(payload)


class TrackAFscilExperimentConfig(BaseModel):
    """Typed configuration for the FSCIL FSCIL runner."""

    task: Literal["track_a_fscil"] = "track_a_fscil"
    experiment_name: str
    method: TrackAFscilMethod
    dataset_config_path: Path = Path("configs/data/ton_iot.yaml")
    train_path: Path
    calibration_path: Path
    test_path: Path
    ...
    @model_validator(mode="after")
    def validate_fscil_policy(self) -> TrackAFscilExperimentConfig:
...
def load_track_a_fscil_config(
    ...
    *,
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"Expected a mapping in {resolved_path}")
    return TrackAFscilExperimentConfig.model_validate(payload)


class CollapseTestsExperimentConfig(BaseModel):
    """Typed configuration for the collapse-test runner."""

    task: Literal["collapse_tests"] = "collapse_tests"
    experiment_name: str
    method: CollapseTestsMethod = "bc_mrnfst"
    dataset_config_path: Path = Path("configs/data/ton_iot.yaml")
    train_path: Path
    calibration_path: Path
    test_path: Path
    ...
    near_collision_threshold: float = 1.5
    synthetic_noise_scale: float = 0.12
    c1_train_rows_per_cluster: PositiveInt = 8
    c1_cal_rows_per_cluster: PositiveInt = 3
    c1_test_rows_per_cluster: PositiveInt = 4
    c2_train_rows_per_anchor: PositiveInt = 6
    c2_cal_rows_per_anchor: PositiveInt = 2
    c2_test_rows_per_anchor: PositiveInt = 3

    monitoring_session_names: list[str] = Field(default_factory=list)
    max_monitoring_sessions: PositiveInt | None = None

    model: BCMRNFSTConfig = Field(default_factory=BCMRNFSTConfig)

    @field_validator("near_collision_threshold", "synthetic_noise_scale")
    @classmethod
    def validate_positive_float(cls, value: float) -> float:
        """Require positive collapse-test scalar settings."""
...
class FidelityStudyExperimentConfig(BaseModel):
    """Typed configuration for the fidelity-study runner."""

    task: Literal["fidelity_study"] = "fidelity_study"
    experiment_name: str
    method: FidelityStudyMethod = "bc_mrnfst"
    dataset_config_path: Path = Path("configs/data/ton_iot.yaml")
    train_path: Path
    calibration_path: Path
    test_path: Path
    ...
    @model_validator(mode="after")
    def validate_holdout_policy(self) -> FidelityStudyExperimentConfig:
        """Require at least one way to generate holdout episodes."""
    ...
    def to_comparison_baseline_config(
        self,
        method: BaselineKind,
    ) -> BaselineExperimentConfig:
        """Adapt the fidelity-study config into the shared baseline fit config."""

        if method != "batch_knfst":
            raise ValueError(
                "The fidelity-study runner supports only the batch_knfst comparison baseline."
            )
        return BaselineExperimentConfig(
            experiment_name=self.experiment_name,
            method=method,
...
def load_collapse_tests_config(
    path: Path,
    *,
    repo_root: Path | None = None,
) -> CollapseTestsExperimentConfig:
    """Load a collapse-test config from YAML."""

    resolved_path = path if path.is_absolute() else (repo_root or find_repo_root()) / path
    payload = yaml.safe_load(resolved_path.read_text(encoding="utf-8")) or {}
    if not isinstance(payload, dict):
        raise ValueError(f"Expected a mapping in {resolved_path}")
    return CollapseTestsExperimentConfig.model_validate(payload)


def load_fidelity_study_config(
    path: Path,
    *,
    repo_root: Path | None = None,
) -> FidelityStudyExperimentConfig:
    """Load a fidelity-study config from YAML."""

    resolved_path = path if path.is_absolute() else (repo_root or find_repo_root()) / path
    payload = yaml.safe_load(resolved_path.read_text(encoding="utf-8")) or {}
    if not isinstance(payload, dict):
        raise ValueError(f"Expected a mapping in {resolved_path}")
    return FidelityStudyExperimentConfig.model_validate(payload)
