...
from bcmrnfst.config.data import PreprocessingConfig
from bcmrnfst.maps.explicit import ExplicitMapConfig, LandmarkStrategy, MapKind
...
class BCMRNFSTConfig(BaseModel):
    ...
    projection_mode: Literal[
        "null_space", "pca", "random", "dvmad_two_tailed"
    ] = "null_space"
    dvmad_eps: float = 0.1
    dvmad_artificial_mode: Literal["max", "min", "farthest"] = "farthest"
    enable_dvmad_gate: bool = False
    dvmad_quantile: float = 0.90
    dvmad_gate_use_phi: bool = True
    dvmad_gate_mode: Literal["both", "min", "max"] = "both"
    dvmad_gate_policy: Literal["or", "borderline", "blend"] = "or"
    dvmad_gate_borderline_margin: float = 0.2
    dvmad_gate_blend_alpha: float = 0.5
    dvmad_gate_min_class_support: int = 6
    enable_pct_rd: bool = False
    pct_rd_min_per_class: int = 4
    enable_carag: bool = False
    carag_buffer_cap: int = 256
    carag_use_anchor_blend: bool = True
    carag_min_anchor_support: int = 10
    carag_ambiguity_lo: float = 0.85
    carag_ambiguity_hi: float = 1.0
    enable_ewm: bool = False
    ewm_alpha_mix: float = 1.0
    ewm_pca_whiten: bool = False
    ewm_pca_dim: int = 64
    ewm_replace_gate: bool = False
    ewm_and_combine_gate: bool = False
    ewm_whiten_mode: Literal["none", "pca", "sw"] = "none"
    ewm_sw_reg: float = 1e-3
    # --- NSD-Gate: re-expressed Sw-EWM gate via null-space + Fisher weights.
    enable_nsd_gate: bool = False
    nsd_keep_residual: bool = True   # psi=1 (two-tail); False=psi=0 ablation-only.
    nsd_use_sw_metric: bool = True   # gate on Sw-whitened features.
    nsd_eps_split: float = 1e-3      # null (lam<eps) / saturated (lam>1-eps) spectrum split.
    nsd_clip_omega_n: float | None = None  # optional Omega_N condition-number clip (default off).
