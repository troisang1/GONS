...
The GONS code path imports these symbols only for type annotations
(`BaselineModel` participates in the `ModelState` union) and for the
...
`gons.baselines.config` (baseline config *schema* only — no method code) is
retained intact because `gons.evaluation.config` imports its dataclasses.
"""
...
_EXCLUDED = (
    ...
    "This release contains only GONS."
)


@dataclass
class BaselineModel:
    """Inert placeholder so the `ModelState` type union resolves.
